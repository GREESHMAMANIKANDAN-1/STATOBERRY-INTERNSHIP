...
>>>>>>> 997eec17a64ad5eefb311c8eb077b4e101bfebaf
...
<<<<<<< HEAD
![Analysis of experiment tab](www/PLACEHOLDER_FrontPage_2FRBD.webp){#fig-aov fig-align="center"}
=======
...
::: {style="text-align: justify;"}
To make things simple and interesting, we'll explain 2F-RBD analysis step by step using a working example, so you can clearly see how it works and why it matters. This dataset represents a factorial experiment involving two factors: **FactorA** and **FactorB**, each with multiple levels, laid out in a Randomised Block Design. FactorA consists of **[PLACEHOLDER: number of levels, e.g., 2 levels (a1, a2)]**, while FactorB has **[PLACEHOLDER: number of levels, e.g., 3 levels (b1, b2, b3)]**. The experiment is conducted in a Randomised Block Design (RBD), where the **[PLACEHOLDER: number of treatment combinations]** treatment combinations formed by crossing the levels of FactorA and FactorB are randomly assigned within each of **[PLACEHOLDER: number of blocks, e.g., 4 blocks]**. Observations were collected for **[PLACEHOLDER: number and names of response variables, e.g., 6 response variables: y1, y2, y3, y4, y5, and y6]**, representing different measured characteristics. **[PLACEHOLDER: Add a brief description of what the factors and variables represent in the context of the experiment, e.g., Factor A represents irrigation levels, Factor B represents fertiliser doses, and variables represent yield and quality traits.]** This dataset allows for the examination of the individual effects of FactorA and FactorB, as well as their interaction effects, with block effects accounted for to improve precision. Our aim is to test whether the factors and their interaction produce statistically significant differences in the response variables using ANOVA. The arrangement of the data is shown in the @fig-data.
:::

<!-- REPLACE THIS SCREENSHOT -->
![Example dataset](www/PLACEHOLDER_ExcelData_2FRBD.webp){#fig-data fig-align="center"}

::: {style="text-align: justify;"}
Data organized in MS Excel can be directly uploaded to **RAISINS** for analysis. For more details on data preparation see @sec-4. Three terms that we will use frequently are **Factor A**, **Factor B**, and **Variables**. In our example, Factor A has **[PLACEHOLDER: levels]**, Factor B has **[PLACEHOLDER: levels]**, and the Variables are **[PLACEHOLDER: list of traits/response variables]**.
:::
...
<!-- REPLACE THIS SCREENSHOT -->
<<<<<<< HEAD
![Illustrating how to create a dataset](www/PLACEHOLDER_CreateData_2FRBD.webp){#fig-4 fig-align="center"}
=======
...
<!-- REPLACE THIS SCREENSHOT -->
<<<<<<< HEAD
![Analysis window explained](www/PLACEHOLDER_AnalysisTab_2FRBD.webp){#fig-5 fig-align="center"}
=======
...
::: {style="text-align: justify;"}
Once your dataset is uploaded, click on `Run Analysis`, **two-way ANOVA** (factorial ANOVA) will be performed within a randomised block framework. Analysis of Variance **(ANOVA)** is a statistical technique used to test the significance of differences among population means by partitioning total variation into Blocks, Factor A, Factor B, Interaction (A × B), and error components and comparing them using the F-test.
:::
...
<!-- REPLACE THIS SCREENSHOT -->
![Mean squares and significance](www/PLACEHOLDER_ANOVATable1_2FRBD.webp){fig-align="center"}
...
::: {style="text-align: justify;"}
**[PLACEHOLDER: Add interpretation of the ANOVA summary table here. Describe which factors (Factor A, Factor B) and/or their interaction (A × B) are significant or non-significant for each response variable. Mention the error mean squares, degrees of freedom, and any patterns observed across traits. For example: "The ANOVA summary indicates that Factor A shows a significant effect on y1 and y3 (p < 0.05), while Factor B is non-significant across all traits. The interaction A × B is significant for y2 at the 5% level, suggesting that the combined effect of the two factors influences y2 beyond their individual effects. The error mean squares range from X to Y with Z degrees of freedom."]**
:::
...
<!-- REPLACE THIS SCREENSHOT -->
![Factor A detailed results with multiple comparisons](www/PLACEHOLDER_ANOVATable2_FactorA_2FRBD.webp){fig-align="center"}

**Table 3: Factor B - Detailed results with multiple comparisons**

<!-- REPLACE THIS SCREENSHOT -->
![Factor B detailed results with multiple comparisons](www/PLACEHOLDER_ANOVATable3_FactorB_2FRBD.webp){fig-align="center"}

**Table 4: Interaction (A × B) - Detailed results**

<!-- REPLACE THIS SCREENSHOT -->
![Interaction (A × B) detailed results](www/PLACEHOLDER_ANOVATable4_Interaction_2FRBD.webp){fig-align="center"}
...
::: {style="text-align: justify;"}
**[PLACEHOLDER: Add detailed interpretation of Tables 2, 3, and 4 here. For each factor (A and B) and the interaction (A × B), describe the mean values at each level, the significance of F-statistics, p-values, and any pairwise groupings indicated by letter notation. Comment on the critical difference (CD) values, MSE, CV(%), and Cohen's F for each trait. For example: "For Factor A, the mean values across levels a1 and a2 show meaningful differences for y1 (F = X.XX, p = 0.0X), with the LSD groupings confirming that a2 (mean = X.XX, group 'a') is significantly higher than a1 (mean = X.XX, group 'b'). For Factor B, ... The interaction A × B reveals significant combined effects for ... The CV(%) values ranging from X to Y suggest moderate/low experimental variability, indicating good precision in the blocking arrangement."]**
:::
...
<!-- REPLACE THIS SCREENSHOT -->
![Multiple comparison test options explained](www/PLACEHOLDER_AnalysisTab_2FRBD.webp){#fig-7 fig-align="center"}
=======
After obtaining a significant F-value in ANOVA under 2F-RBD, multiple comparison tests are employed to identify which factor level means differ significantly. In a factorial design with a block arrangement, post hoc comparisons can be applied separately for each significant factor (Factor A, Factor B) and for the interaction (A × B) when significant. Commonly used post hoc tests include Least Significant Difference (LSD), Tukey's Honest Significant Difference (HSD), and Duncan's Multiple Range Test (DMRT), each differing in their level of error control and suitability depending on the number of treatments and experimental conditions,see @fig-7.
:::
...
The **Least Significant Difference (LSD)** test is a post-hoc statistical procedure used to identify which specific factor level means differ significantly after ANOVA has indicated a significant effect. The LSD is calculated as $$
...
After confirming significant overall differences via ANOVA, DMRT ranks the factor level means and calculates critical differences using the studentized range statistic (Q) and the standard error based on error variance from ANOVA. This systematic and sequential approach provides clear groupings of factor levels and has gained popularity in agricultural research. </small>
...
<!-- REPLACE THIS SCREENSHOT -->
![Plot settings for customizing](www/PLACEHOLDER_BasicPlots_Customize_2FRBD.webp){#fig-8 fig-align="center"}
...
::: {style="text-align: justify;"}
**[PLACEHOLDER: Describe the box plot here. Mention the factor being plotted, the distribution of the response variable across factor levels, approximate median values, spread/variability, outliers if any, and the letter groupings confirming statistical significance between levels.]**
=======
...
::: {style="text-align: justify;"}
**[PLACEHOLDER: Describe the violin plot here. Discuss the distribution shape for each factor level, the concentration of data, the width of each violin indicating density, and any differences in spread or central tendency between levels. Include letter groupings if applicable.]**
=======
...
::: {style="text-align: justify;"}
**[PLACEHOLDER: Describe the bar plot here. Compare the mean values of the response variable across factor levels with error bars. Mention the approximate mean values for each level, whether error bars overlap, and the letter groupings confirming statistical differences as determined by the LSD/Tukey/DMRT test.]**
=======

![Bar plot](images/bar plot.png){#fig-11}
...
::: {style="text-align: justify;"}
**[PLACEHOLDER: Describe the mean value plot here. Explain the horizontal line representation of means for each factor level, the approximate positions of each level's mean on the axis, and the letter groupings shown to confirm significant differences between factor levels.]**
=======

![Mean Value Plot](images/mean value.png){#fig-12}
...
<!-- REPLACE THIS SCREENSHOT -->
<<<<<<< HEAD
![Connected line plot](www/PLACEHOLDER_ConnectedLinePlot_2FRBD.webp){#fig-13}
=======
...
::: {style="text-align: justify;"}
<<<<<<< HEAD
**[PLACEHOLDER: Describe the connected line plot here. Explain how the mean values for each factor level are connected by a line to highlight the trend across levels. Mention the direction of the trend (increasing/decreasing), approximate mean values, error bars, and the letter groupings confirming statistical differences.]**
=======
...
<!-- REPLACE THIS SCREENSHOT -->
<<<<<<< HEAD
![Advanced plots](www/PLACEHOLDER_AdvancedTab_2FRBD.webp){#fig-90 fig-align="center"}
=======
...
<!-- REPLACE THIS SCREENSHOT -->
![INTERACTION plot](www/PLACEHOLDER_InteractionPlot_2FRBD.webp){#fig-14 fig-align="center"}
...
<!-- REPLACE THIS SCREENSHOT -->
![3D SCATTER plot](www/PLACEHOLDER_3DScatter_2FRBD.gif){#fig-15 fig-align="center"}
...
<!-- REPLACE THIS SCREENSHOT -->
![3D SCATTER PLOT WITH LINE](www/PLACEHOLDER_3DScatterLine_2FRBD.gif){#fig-16 fig-align="center"}
...
<!-- REPLACE THIS SCREENSHOT -->
<<<<<<< HEAD
![Multivariate tab in RAISINS](www/PLACEHOLDER_Multivariate_2FRBD.webp){#fig-mu}
=======
...
<!-- REPLACE THIS SCREENSHOT -->
<<<<<<< HEAD
![Manova Results](www/PLACEHOLDER_MANOVA_2FRBD.webp){#fig-MAN2 fig-align="center"}
=======
...
<!-- REPLACE THIS SCREENSHOT -->
![PCA-based Index Score](www/PLACEHOLDER_PCA_2FRBD.webp){#fig-PC}
...
<!-- REPLACE THIS SCREENSHOT -->
![Scree plot](www/PLACEHOLDER_Scree_2FRBD.webp){#fig-screeplot fig-align="center"}
...
<!-- REPLACE THIS SCREENSHOT -->
![PCA-Biplot](www/PLACEHOLDER_Biplot_2FRBD.webp){#fig-biplot}
...
<!-- REPLACE THIS SCREENSHOT -->
![Index plot](www/PLACEHOLDER_IndexPlot_2FRBD.webp){#fig-index fig-align="center"}
...
<!-- REPLACE THIS SCREENSHOT -->
<<<<<<< HEAD
![Creating dataset within RAISINS](www/PLACEHOLDER_CreateData_2FRBD.webp){#fig-createdata}
=======
...
<!-- REPLACE THIS SCREENSHOT -->
<<<<<<< HEAD
![Model dataset](www/PLACEHOLDER_ModelDataset_2FRBD.webp){#fig-188 fig-align="center"}
=======
...
<!-- REPLACE THIS SCREENSHOT -->
![FAQs](www/PLACEHOLDER_FAQ_2FRBD.webp){#fig-148 fig-align="center"}
...
<!-- REPLACE THIS SCREENSHOT -->
<<<<<<< HEAD
![View data](www/PLACEHOLDER_ViewData_2FRBD.webp){fig-align="center"}
=======
