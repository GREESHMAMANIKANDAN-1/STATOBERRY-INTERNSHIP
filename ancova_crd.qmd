...
![Analysis of experiment tab](www/PLACEHOLDER_AnalysisTab_ANCOVCRD.webp){#fig-aov fig-align="center"}
...
![ANCOVA CRD Layout](www/PLACEHOLDER_Layout_ANCOVCRD.webp){#fig-lay fig-align="center"}
...
::: {style="text-align: justify;"}
To make things simple and interesting, we'll explain **ANCOVA CRD** step by step using a hypothetical example, so you can clearly see how it works and why it matters. Consider a greenhouse experiment conducted to evaluate the effect of **5 fertiliser treatments** on the final biomass of sunflower plants — T1 (Control — no fertiliser), T2 (N alone at 60 kg/ha), T3 (P alone at 40 kg/ha), T4 (N+P combined), and T5 (N+P+K combined) — arranged as a CRD with **4 replications** per treatment, giving a total of 5 × 4 = **20 experimental pots**. Because seedlings of different sizes were used, **initial plant height (cm)** at the time of transplanting was recorded as the **covariate (X)** to account for pre-existing variability among plants. The response variables measured at harvest were: **Biomass** (total fresh biomass in g), **Obs1** (leaf area index), **Obs2** (stem diameter in mm), and **FW** (root fresh weight in g). Our aim is to test whether the five fertiliser treatments produce statistically significant differences in the response variables after adjusting for initial plant height, and to obtain adjusted treatment means for fair comparison. The arrangement of the data is shown in @fig-data.
:::

<!-- REPLACE THIS SCREENSHOT -->

![Example dataset](www/PLACEHOLDER_ExampleDataset_ANCOVCRD.webp){#fig-data fig-align="center"}

::: {style="text-align: justify;"}
Data organized in MS Excel can be directly uploaded to **RAISINS** for analysis. For more details on data preparation see @sec-4. Two terms that we will use frequently are **Treatments** and **Variables**. In our example, the Treatments refer to the 5 fertiliser levels, the covariate is the initial plant height recorded before treatment application, and the Variables are the four response traits measured at harvest — **Biomass, Obs1, Obs2, and FW**.
:::

## How to prepare your data? {#sec-4 .H}

::: {style="text-align: justify;"}
Arranging data for uploading in **RAISINS** is very simple. Prepare your data exactly like the one shown in @fig-data, using a single-sheet Excel file. The file must contain a column for **Treatment** (the fertiliser label for each observation), a column for the **Covariate (X)** (e.g., initial plant height), and separate columns for each response variable. Make sure no blank rows are left above, and all columns have proper names. That's it — your file is ready to upload.
...
![Analysis window explained](www/PLACEHOLDER_AnalysisWindow_ANCOVCRD.webp){#fig-5 fig-align="center"}
...
![Adjusted ANOVA table — Covariate, Treatment (adjusted), and Error (adjusted) Mean Squares](www/PLACEHOLDER_ANOVASummary_ANCOVCRD.webp){#fig-100 fig-align="center"}
...
![Adjusted treatment means with multiple comparisons](www/PLACEHOLDER_DetailedResults_ANCOVCRD.webp){#fig-101 fig-align="center"}
...
![Multiple comparison test options explained](www/PLACEHOLDER_MCT_ANCOVCRD.webp){#fig-7 fig-align="center"}
...
::: {style="text-align: justify;"}
If you need to know about the detailed values of the various statistical metrics of treatments — both unadjusted and adjusted — move to `Summary stats` under the `Analysis`. In the context of **ANCOVA CRD**, the summary statistics table presents both the raw observed means and the covariate-adjusted means alongside their associated precision measures, allowing a direct comparison of what changes when the covariate is accounted for.
:::
...
![Summary statistics](www/PLACEHOLDER_SummaryStats_ANCOVCRD.webp){#fig-102 fig-align="center"}
...
**Mean** The arithmetic average of all raw (unadjusted) observations within a treatment group. Useful for describing the observed data but should not be used for treatment comparisons in ANCOVA without adjustment.

**Adjusted Mean** The estimated treatment mean after removing the linear effect of the covariate, calculated at the grand mean of the covariate. This is the primary quantity of interest in ANCOVA and should be used for all treatment comparisons and recommendations.

**SD (Standard Deviation)** A measure of the amount of variation or dispersion of the raw observations within a treatment group.

**SE (Standard Error)** Specifically the Standard Error of the adjusted Mean. It estimates how far the adjusted sample mean is likely to be from the true population adjusted mean. $$SE = \frac{\text{Standard deviation (SD)}}{\sqrt {n}}*100$$

**Min / Max** The lowest and highest recorded raw values within that specific treatment group.

**CV (Coefficient of Variation)** The ratio of the standard deviation to the raw mean, expressed as a percentage $$CV = \frac{\text{Standard deviation (SD)}}{\text{Mean}}*100$$

**Skewness** A measure of the asymmetry of the probability distribution of the raw observations. Positive value: Data is skewed to the right. Negative value: Data is skewed to the left.
...
::: {style="text-align: justify;"}
The summary statistics table shows both raw and adjusted means for the five fertiliser treatments across 4 replications. Treatment T5 (N+P+K combined) recorded the highest raw mean biomass (328.50 g) and also the highest adjusted mean (319.75 g), confirming its superiority even after correcting for initial plant height differences. Treatment T1 (Control) recorded the lowest raw mean (198.30 g) and the lowest adjusted mean (207.10 g). Notably, the ranking of T2 (N alone) and T3 (P alone) changes slightly between raw and adjusted means — T3 moves above T2 after covariate adjustment — demonstrating the importance of using adjusted means for fair treatment comparisons when initial plant sizes differed systematically among groups. The CV% of 5.3% based on the adjusted error confirms good experimental precision. Skewness values close to zero for all treatments confirm approximate normality of the raw data, supporting the parametric ANCOVA.
:::

## Individual ANOVA {#IA}

::: {style="text-align: justify;"}
If the user wants to get individual ANCOVA tables for each response variable separately, click on `Individual ANOVA` in the `Analysis`. Each table presents the full ANCOVA decomposition — covariate, adjusted treatment, and adjusted error — for one response variable at a time, together with the adjusted means, CD, CV%, MSE, SE(m), and SE(d) as in @fig-104.
:::

**Table 4: ANCOVA table for Biomass**

<!-- REPLACE THIS SCREENSHOT -->

![ANCOVA table for Biomass](www/PLACEHOLDER_IndividualANOVA_ANCOVCRD.webp){#fig-104 fig-align="center"}
...
**Critical Difference (CD)** The minimum difference required between any two adjusted treatment means to consider them significantly different from each other, computed using the adjusted error mean square. At a 1% level, you are 99% confident in the difference. At a 5% level, you are 95% confident.

**Coefficient of Variation (CV (%))** A relative measure of dispersion computed from the adjusted error mean square relative to the grand mean. $$CV = \frac{\sqrt{\text{MSE}_{\text{adj.}}}}{\text{Grand Mean}}*100$$

**Mean Square Error (MSE)** The Adjusted Error Mean Square from the ANCOVA table. A smaller adjusted MSE compared to the unadjusted ANOVA MSE confirms that the covariate reduced the error variance effectively.

**Standard Error of Adjusted Mean (SE(m))** Measures how much the adjusted treatment mean is likely to vary from the true population adjusted mean. $$ SE(m)=\sqrt{MSE_{\text{adj.}} / r}$$ Where r is the number of replications.

**Standard Error of Difference (SE(d))** The standard error associated with the difference between two adjusted treatment means. $$ SE(d)=\sqrt{2 \times MSE_{\text{adj.}} / r}$$
...
::: {style="text-align: justify;"}
For the character **Biomass**, the covariate (initial plant height) mean square is 2,418.65 and highly significant, confirming effective adjustment. The adjusted Treatment mean square is 1,842.30 and the adjusted Error mean square is 131.92, yielding an F-value of approximately 13.97 with 4 degrees of freedom for adjusted treatment and 14 degrees of freedom for adjusted error, significant at the 1% level. The adjusted MSE of 131.92 is considerably smaller than the unadjusted ANOVA error mean square (estimated at 248.60), confirming that including the covariate nearly halved the error variance and substantially improved the precision of the experiment. The SE(m) is approximately 5.74 and the SE(d) is approximately 8.12. The CD at 5% is 17.32 and at 1% is 24.18, indicating the minimum adjusted biomass difference required to declare any two treatments significantly different. The overall CV% of 5.3% based on the adjusted error confirms excellent experimental precision.
:::

**Table 5: Adjusted means with letter groupings**

<!-- REPLACE THIS SCREENSHOT -->

![Adjusted means with groupings](www/PLACEHOLDER_SummaryGroupings_ANCOVCRD.webp){#fig-103 fig-align="center"}

### Interpretation from @fig-103

::: {style="text-align: justify;"}
The adjusted treatment means with letter groupings show that T5 (N+P+K combined) recorded the highest adjusted biomass (319.75 g) and was assigned the letter grouping **"a"**, indicating it is statistically superior to all other treatments after covariate adjustment. It was statistically at par with T4 (N+P combined), which carried the grouping **"ab"**, reflecting a marginal but non-significant difference between these two highest-performing fertiliser combinations. T1 (Control) recorded the lowest adjusted mean biomass (207.10 g) and was assigned the letter grouping **"d"**, confirming it was significantly inferior to all fertilised treatments. Treatments T2 and T3 carried groupings **"bc"** and **"c"** respectively, differing significantly from the top group. These groupings are based entirely on adjusted means and provide a fair, covariate-corrected basis for fertiliser recommendations under the experimental conditions.
:::
...
![Box plot](www/PLACEHOLDER_BoxPlot_ANCOVCRD.webp){#fig-9}
...
![Violin plot](www/PLACEHOLDER_ViolinPlot_ANCOVCRD.webp){#fig-10}
...
![Bar plot](www/PLACEHOLDER_BarPlot_ANCOVCRD.webp){#fig-11}
...
![Mean Value Plot](www/PLACEHOLDER_MeanPlot_ANCOVCRD.webp){#fig-12}
...
![Connected line plot](www/PLACEHOLDER_ConnectedPlot_ANCOVCRD.webp){#fig-13}
:::
...
::: {style="text-align: justify;"}
**RAISINS** also provides `advanced plots` which go beyond basic bar charts and histograms to give deeper insight into your ANCOVA data, especially the covariate-response relationship, residual distributions, and deviations from model assumptions. (See @fig-90)
:::

<!-- REPLACE THIS SCREENSHOT -->

![Advanced plots](www/PLACEHOLDER_AdvancedPlots_ANCOVCRD.webp){#fig-90 fig-align="center"}

**SCATTER PLOT WITH REGRESSION LINES**

<!-- REPLACE THIS SCREENSHOT -->

![Scatter plot with within-group regression lines](www/PLACEHOLDER_ScatterRegression_ANCOVCRD.webp){#fig-14 fig-align="center"}

::: {style="text-align: justify;"}
A scatter plot with within-group regression lines displays the covariate (X) on the horizontal axis and the response variable (Y) on the vertical axis, with each treatment group shown in a distinct colour. A separate regression line is fitted for each group, and their approximate parallelism visually confirms the homogeneity of regression slopes assumption — the foundation of valid ANCOVA. Converging or crossing lines would signal a significant Treatment × Covariate interaction and would caution against standard ANCOVA interpretation.
:::
...
![Adjusted means plot with confidence intervals](www/PLACEHOLDER_AdjustedMeansPlot_ANCOVCRD.webp){#fig-15 fig-align="center"}

::: {style="text-align: justify;"}
An adjusted means plot displays the covariate-corrected treatment means with their confidence intervals side by side, allowing a clear visual comparison of treatment performance after accounting for the covariate. Unlike a simple bar chart of raw means, this plot accurately represents the ANCOVA results and can be used directly in publications to communicate adjusted treatment differences.
:::
...
![QQ plot of adjusted residuals](www/PLACEHOLDER_QQPlot_ANCOVCRD.webp){#fig-16 fig-align="center"}

::: {style="text-align: justify;"}
Q–Q (quantile–quantile) plot of the ANCOVA adjusted residuals is used to assess the normality assumption. The points represent observed residual quantiles plotted against theoretical quantiles from a normal distribution. Because most points lie close to the diagonal reference line, the adjusted residuals approximately follow a normal distribution, confirming the validity of the parametric ANCOVA and the reliability of the F-tests and adjusted means.
:::
...
![Residual vs. fitted values plot](www/PLACEHOLDER_ResidualPlot_ANCOVCRD.webp){#fig-17 fig-align="center"}

::: {style="text-align: justify;"}
A residual plot displays the ANCOVA adjusted residuals on the vertical axis against the fitted (predicted) values on the horizontal axis. A random scatter of residuals with no systematic pattern confirms that the linearity and homoscedasticity assumptions of ANCOVA are met. Patterns such as a fan shape or curved trend would indicate heteroscedasticity or non-linearity, respectively, signalling the need for data transformation or a non-linear model.
:::
...
![Distribution plot of adjusted residuals](www/PLACEHOLDER_Distribution_ANCOVCRD.webp){#fig-18 fig-align="center"}

::: {style="text-align: justify;"}
The distribution plot shows how the ANCOVA adjusted residuals are distributed. It provides a visual summary of the spread and symmetry of the residuals across all treatment groups, helping to confirm that the error distribution is approximately normal and that no single treatment group contributes disproportionate variability to the adjusted error term.
:::
...
![Multivariate tab in RAISINS](www/PLACEHOLDER_MultivariateTab_ANCOVCRD.webp){#fig-mu}

::: {style="text-align: justify;"}
MANOVA and PCA will be automatically carried out based on the selected response variables using the ANCOVA-adjusted dataset. MANOVA table with interpretation appears automatically. PCA results and plots will appear along with automated interpretation.
:::

<!-- REPLACE THIS SCREENSHOT -->

![Manova Results](www/PLACEHOLDER_MANOVAResults_ANCOVCRD.webp){#fig-MAN2 fig-align="center"}

::: {style="text-align: justify;"}
The table titled 'Eigen Values PCA' given @fig-PC provides information about the eigen values and the percentage of variance explained by each principal component derived from the ANCOVA-adjusted data. The principal components PC1 and PC2 have eigenvalues greater than one and are considered important for further analysis. PC1 accounts for approximately 59% of the variance in the dataset, while PC2 accounts for about 20% of the variance. Together, PC1 and PC2 explain approximately 79% of the total variance (termed as cumulative variance). Since PC1 explains more than 40% of the variance, a PC1-based index score is a strong consideration. Additionally, since both PCs together explain more than 60% of the variance in the data, an index score based on both PCs is also appropriate. The scree plot below illustrates the proportion of variance explained by each principal component.
:::

<!-- REPLACE THIS SCREENSHOT -->

![PCA-based Index Score](www/PLACEHOLDER_PCAIndex_ANCOVCRD.webp){#fig-PC}
...
![Scree plot](www/PLACEHOLDER_ScreePlot_ANCOVCRD.webp){#fig-screeplot fig-align="center"}
...
![Loadings of each variable](www/PLACEHOLDER_Loadings_ANCOVCRD.webp){#fig-loadings fig-align="center"}
...
![PCA-Biplot](www/PLACEHOLDER_Biplot_ANCOVCRD.webp){#fig-biplot}
...
![Index score](www/PLACEHOLDER_IndexScore_ANCOVCRD.webp){#fig-indexscore fig-align="center"}

<!-- REPLACE THIS SCREENSHOT -->

![Indexplot](www/PLACEHOLDER_IndexPlot_ANCOVCRD.webp){#fig-index fig-align="center"}

::: {style="text-align: justify;"}
Combining all this information, the researcher can arrive at an overall conclusion that is statistically sound and contextually relevant to their study. The integration of ANCOVA-adjusted treatment comparisons with multivariate PCA-based index scores enables agronomists and plant scientists to identify the fertiliser treatment that is not only significantly superior in individual adjusted traits but also performs best across multiple response variables simultaneously — accounting for initial plant size differences that would otherwise confound the comparison.
:::
