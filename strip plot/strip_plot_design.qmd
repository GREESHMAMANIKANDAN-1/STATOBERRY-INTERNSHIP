...
::: {style="text-align: justify;"}
To make things simple and interesting, we'll explain Strip Plot Design analysis step by step using a hypothetical example, so you can clearly see how it works and why it matters.A working example of the given figure can be explained using a Strip Plot Design experiment conducted to study the combined effect of **irrigation levels** and **rice varieties** on yield and related traits. In this experiment, Factor A (row treatments) consists of different irrigation levels such as continuous flooding, alternate wetting and drying, and deficit irrigation, while Factor B (column treatments) includes rice varieties like IR64, Swarna, ADT45, and Jyothi. The experiment is arranged in several replications to improve accuracy. Within each replication, the field is divided into horizontal strips representing irrigation levels and vertical strips representing varieties, and their intersections form individual plots where observations are recorded. For example, in Replication 1, each combination of **irrigation level** (R1, R2, etc.) and **variety** (C1, C2, C3, etc.) is measured for variables such as **yield, Panicle length, number of spikelets, and grain weight** (shown as y1, y2, y3… values). This structure is repeated across replications (Replication 2, 3, and 4), ensuring that every treatment combination appears in each block. The collected data are then analyzed using ANOVA to determine whether irrigation levels, varieties, and their interaction have a significant effect on crop performance. This layout helps in clearly separating and evaluating the main and interaction effects of both factors in an efficient and systematic manner. @fig-data.
:::
...
![Multiple comparison results for Strip Plot Design](images/plot design.png){#fig-MCT fig-align="center"}
...
![Multivariate tab in RAISINS](images/Screenshot 2026-04-30 220202.png){#fig-mu}
...
![FAQs](images/Screenshot 2026-04-30 220430.png){#fig-148 fig-align="center"}
