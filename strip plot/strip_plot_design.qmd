...
::: {style="text-align: justify;"}
To get started, visit **RAISINS** [www.raisins.live](https://www.raisins.live) home page and go to **Analysis of experiments**. Here, you can see different experimental designs available on the platform. In this tutorial, we focus on **Strip Plot Design (SPD)**, as shown in @fig-aov.
:::
...
::: {style="text-align: justify;"}
A Strip Plot Design (SPD) is a two-factor experimental layout in which the levels of one factor (called the **horizontal strip factor** or Factor A) are assigned to horizontal strips running across each block, while the levels of a second factor (called the **vertical strip factor** or Factor B) are assigned to vertical strips running down each block. Each block is thereby divided into a grid of interaction plots, where every level of Factor A intersects every level of Factor B exactly once. This arrangement is replicated across two or more blocks, and treatments within each factor are randomised independently within each block. SPD is particularly suited to experiments where one or both factors require large, contiguous areas for mechanised application — such as irrigation regimes, tillage depths, or sowing methods — making it impractical to use a fully randomised split-plot or factorial arrangement. The design produces three separate error terms: one for the horizontal factor, one for the vertical factor, and one for their interaction, providing the highest precision for the interaction effect. When factors do not require large-scale application equipment and experimental units are relatively homogeneous, a Randomised Block Design (RBD) or Split Plot Design may be more appropriate.
:::
...
::: callout-tip
#### Strip Plot Design (SPD) is a two-factor experimental design where one factor is applied in horizontal strips and another in vertical strips across each block, producing three independent error terms and maximum precision for the interaction between the two factors.
:::

## A working example {#W}

::: {style="text-align: justify;"}
To make things simple and interesting, we'll explain SPD analysis step by step using a hypothetical example, so you can clearly see how it works and why it matters. Let's assume an experiment is conducted to evaluate the combined effect of **irrigation levels** (Factor A, horizontal strips) and **rice varieties** (Factor B, vertical strips) on grain yield and related traits. The experiment is replicated across three blocks. Factor A comprises three irrigation levels: **I1** (continuous flooding), **I2** (alternate wetting and drying), and **I3** (deficit irrigation). Factor B comprises four rice varieties: **V1** (IR64), **V2** (Swarna), **V3** (ADT45), and **V4** (Jyothi). Observations were recorded for four variables: **Yield, Panicle_length, Spikelet_number, and Grain_weight**. Our aim is to test whether the main effects of irrigation level, variety, and their interaction produce statistically significant differences in the response variables using ANOVA. The arrangement of the data is shown in @fig-data.
:::
...
::: {style="text-align: justify;"}
Data organised in MS Excel can be directly uploaded to **RAISINS** for analysis. For more details on data preparation see @sec-4. Two terms that we will use frequently are **Treatments** and **Variables**. In our example, the Treatments refer to the combinations of irrigation level and rice variety, and the Variables are the four traits mentioned earlier — **Yield, Panicle_length, Spikelet_number, and Grain_weight**.
:::
...
![SPD analysis window explained](images/Copy%20of%20Copy%20of%20Red%20Modern%20Blank%20Page%20A4%20Landscape-01.png){#fig-5 fig-align="center"}
...
::: {style="text-align: justify;"}
Log, square root, and arcsine transformations are often used in SPD analysis to make data more normal and reduce uneven variation. Researchers can use these transformations when analysing experimental data in **RAISINS** as shown in @fig-6.
:::
...
@sec-8 provides detailed information on the multiple comparison tests (post-hoc tests) available in **RAISINS** for SPD.
:::
...
**Interaction (A × B)**: The combined effect of Factor A and Factor B applied simultaneously in each interaction subplot. Tested against Error(ab), the smallest and most precise error term in SPD.
...
**MSE (Mean Square Error)**: The residual variance from the ANOVA. Three MSE values appear in SPD — one for Error(a), one for Error(b), and one for Error(ab).
...
<small> A post-hoc test (also called a multiple comparison test) is a follow-up procedure performed after a significant ANOVA result to determine which specific group means differ from one another. ANOVA only informs you that at least one pair of means is different; it does not identify which pairs. Post-hoc tests correct for the inflated Type I error rate that arises when performing many pairwise comparisons simultaneously. In SPD, post-hoc comparisons are conducted separately for Factor A means (using Error(a)), Factor B means (using Error(b)), and interaction cell means (using Error(ab)), ensuring that each comparison is tested against the appropriate precision. </small>

</details>

::: {style="text-align: justify;"}
**RAISINS** provides three post-hoc tests for SPD analysis: **Fisher's LSD**, **Tukey's HSD**, and **DMRT**. Each test controls Type I error at different levels of stringency, and the appropriate choice depends on the number of comparisons and the researcher's tolerance for false positives.
...
![Multiple comparison results for SPD](images/Copy%20of%20Red%20Modern%20Blank%20Page%20A4%20Landscape%20(13)-02.png){#fig-MCT fig-align="center"}
...
::: {style="text-align: justify;"}
The choice of post-hoc test in SPD depends on the researcher's priorities and the number of treatment levels being compared.

**LSD** is appropriate when comparisons are planned and limited in number, and when sensitivity to real differences is paramount. In agricultural SPD studies with few treatment levels, LSD is the most commonly used test and provides the highest power to detect genuine differences.
...
::: {style="text-align: justify;"}
A **box plot** compares the distribution of values across different treatment combinations in the Strip Plot Design. Each coloured box represents a Factor A × Factor B combination and shows key statistics: the median (middle line), the interquartile range (the box), and potential outliers (points outside the whiskers). Letters above each box indicate statistical groupings — combinations sharing letters are statistically similar, while those with different letters are significantly different.
:::
::::
:::::
::::::
...
::: {style="text-align: justify;"}
A **violin plot** displays the distribution of values for each treatment combination in SPD. The width of the violin at any point represents the density of data at that value, providing richer information about the spread than a simple box plot. The embedded box plot shows the median and interquartile range. Grouping letters indicate which treatment combinations are statistically similar or different following the post-hoc test.
:::
::::
:::::
::::::
...
::: {style="text-align: justify;"}
A **bar plot** compares the average values of treatment combinations across Factor A and Factor B levels, with error bars representing variability. The letters above each bar indicate statistical groupings from the post-hoc test — combinations sharing letters are not significantly different. This plot provides a clear visual summary of mean performance across all Factor A × Factor B combinations and is ideal for publication-quality reporting.
:::
::::
:::::
::::::
...
::: {style="text-align: justify;"}
A **mean value plot** displays the mean values of treatment combinations as coloured dots along a horizontal axis, with error bars indicating variability. Treatment combinations are listed vertically, allowing easy comparison of means across irrigation levels and varieties simultaneously. Letters beside each point show post-hoc groupings, revealing which combinations perform at comparable levels and which are distinctly different.
:::
::::
:::::
::::::
...
::: {style="text-align: justify;"}
A **connected line plot** is particularly useful in Strip Plot Design for visualising interaction patterns. Each line represents a level of Factor A (e.g., an irrigation level), and the x-axis shows levels of Factor B (e.g., varieties). When lines cross or diverge, this is a visual cue for a significant A × B interaction — indicating that the ranking of Factor A levels changes depending on the level of Factor B. This plot is among the most informative visualisations available for two-factor experiments.
:::
::::
:::::
::::::
...
![Summary plot](www/PLACEHOLDER_SummaryPlot_SPD.webp){#fig-AP1 fig-align="center"}

![Summary plot](images/WhatsApp Image 2026-04-29 at 3.06.26 PM (1).jpeg){fig-align="center"}

::: {style="text-align: justify;"}
The **Summary Plot** provides a comprehensive single-panel visualisation of mean values, error bars, and individual data points for all Factor A × Factor B treatment combinations. It combines the information from bar plots and raw data overlays to give a complete picture of treatment performance and variability. This plot is especially useful for quickly identifying which treatment combinations stand out and whether variability is homogeneous across groups.
:::
...
![Advanced Raincloud plot](www/PLACEHOLDER_AdvancedRaincloud_SPD.webp){#fig-AP2 fig-align="center"}

![Advanced Raincloud plot](images/WhatsApp Image 2026-04-29 at 3.06.54 PM.jpeg){fig-align="center"}

::: {style="text-align: justify;"}
The **Advanced Raincloud Plot** integrates a half-violin distribution, a box plot, and individual raw data points into a single composite display for each treatment combination. This multi-layer approach reveals not only the central tendency and spread but also the shape of the distribution and the position of individual observations. In SPD with multiple interaction cell means, this plot enables a nuanced comparison of distributional patterns across Factor A levels within each level of Factor B.
:::
...
![QQ plot](images/ssstrip ppplot-01.gif){#fig-AP3 fig-align="center"}

::: {style="text-align: justify;"}
The **QQ (Quantile-Quantile) Plot** is a diagnostic tool used to assess whether the residuals from the SPD ANOVA follow a normal distribution, which is a key assumption of the analysis. Each point represents a residual plotted against its expected value under normality. Points that closely follow the diagonal reference line indicate that the normality assumption is reasonably satisfied. Systematic departures from the line suggest the need for data transformation prior to analysis.
:::
...
![Rain Cloud plot](www/PLACEHOLDER_RaincloudPlot_SPD.webp){#fig-AP4 fig-align="center"}

![Rain Cloud plot](images/sstrip plott-03.gif){fig-align="center"}

::: {style="text-align: justify;"}
The **Rain Cloud Plot** displays the distribution of raw data as scattered points (the "rain") beneath a half-violin density plot (the "cloud"), combined with a box plot summary. This plot avoids the information loss inherent in bar charts and box plots by showing the actual data points alongside distributional summaries. For SPD, it is particularly effective at illustrating how raw observations are spread within each Factor A or Factor B level, helping to identify any outliers or unusual distributional patterns.
:::
...
![Distribution plot](www/PLACEHOLDER_DistributionPlot_SPD.webp){#fig-AP5 fig-align="center"}

::: {style="text-align: justify;"}
The **Distribution Plot** presents a smoothed kernel density estimate for the response variable within each treatment combination. It provides an alternative to histograms by showing a continuous curve that reflects the probability density of the observed data. Comparing distribution plots across Factor A and Factor B levels allows the researcher to assess whether the spread and shape of the data are consistent across treatment groups, which has implications for the validity of the ANOVA assumptions.
:::

## AI Interpretation {#AI}

::: {style="text-align: justify;"}
**RAISINS** includes a built-in AI Assistant that automatically reads your SPD analysis results and generates a plain-language interpretation. The AI Assistant summarises the significance of Factor A, Factor B, and their interaction, identifies the best-performing treatment combinations based on post-hoc groupings, highlights any notable patterns in the interaction, and suggests appropriate next steps — such as further confirmation trials, transformation checks, or multivariate analysis. This feature saves time and ensures that results are interpreted correctly, even by users who are not specialists in experimental statistics. The AI Interpretation panel is shown in @fig-ai.
:::

![AI Interpretation](images/WhatsApp%20Image%202026-04-29%20at%203.08.26%20PM.jpeg){#fig-ai fig-align="center"}

![AI Interpretation](www/ai.webp){fig-align="center"}
...
![Multivariate tab in RAISINS](images/WhatsApp%20Image%202026-04-29%20at%203.09.03%20PM.jpeg){#fig-mu}
...
![MANOVA Results](){#fig-MAN2 fig-align="center"}
...
![PCA Biplot](www/PLACEHOLDER_Biplot_SPD.webp){#fig-biplot}
...
![Index score](images/WhatsApp Image 2026-04-29 at 4.23.30 PM.jpeg){#fig-indexscore fig-align="center"}

![Index plot](images/WhatsApp Image 2026-04-29 at 4.26.22 PM.jpeg){#fig-index fig-align="center"}
...
![Model-1: showing how the prepared Excel file for upload should look like](images/WhatsApp Image 2026-04-29 at 4.27.45 PM.jpeg){#fig-pp}

![Model-2: Showing how the prepared Excel file for upload should look like](images/WhatsApp Image 2026-04-29 at 4.29.39 PM.jpeg){#fig-kk}
...
![Model dataset](images/WhatsApp Image 2026-04-29 at 4.33.10 PM.jpeg){#fig-188 fig-align="center"}
...
![FAQs](images/WhatsApp Image 2026-04-29 at 3.15.18 PM.jpeg){#fig-148 fig-align="center"}
...
![View data](images/WhatsApp Image 2026-04-29 at 3.13.07 PM.jpeg){fig-align="center"}
